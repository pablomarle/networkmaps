...
class ETMap {
    ...
    processMessage(ws, message) {
        ...
        switch(m.m) {
            ...
            case "D":
                ...
                break;
            case "BG":
                this.processMessage_background(ws, m.d);
                break;
            default:
    ...
    processMessage_move(ws, d) {
        if(d.t == "base") {
            this.moveBase(ws, d.v, d.i, d.x, d.y, d.z);
        }
        else if(d.v == "L2" && d.t == "joint") {
            this.moveL2Joint(ws, d.et, d.i, d.joint_index, d.x, d.y, d.z);
        }
        else if(d.v == "L3" && d.t == "joint") {
            this.moveL3Joint(ws, d.et, d.i, d.joint_index, d.x, d.y, d.z);
        }
        else {
            this.moveElement(ws, d.v, d.t, d.i, d.x, d.y, d.z, d.base);
        }
    }
    processMessage_rotate(ws, d) {
        this.rotateElement(ws, d.v, d.t, d.i, d.x, d.y, d.z);
    }

    processMessage_resize(ws, d) {
        this.resizeElement(ws, d.v, d.t, d.i, d.x, d.y, d.z);
    }

    processMessage_settings(ws, d) {
        if(((d.v === "L2") || (d.v === "L3")) && d.t == "base") {
            this.settingsBase(ws, d.v, d.i, d.name, d.subtype, d.color1, d.color2, d.opacity, d.t1name, d.t2name, d.sy, d.tsx, d.tsy);
        }
        else if(d.v == "L2" && d.t == "device") {
            this.settingsL2Device(ws, d.i, d.name, d.color1, d.color2, d.ifnaming);
        }
        else if(d.v == "L2" && d.t == "link") {
            this.settingsLink(ws, d.v, d.t, d.i, d.type, d.order, d.color, d.weight, d.height);
        }
        else if(d.v == "L3" && d.t == "vrf") {
            this.settingsVrf(ws, d.i, d.color1, d.color2);
        }
        else if(d.v == "L3" && d.t == "l2segment") {
            this.settingsL2Segment(ws, d.i, d.color1);
        }
        else if(d.v == "L3" && ((d.t == "l2link") || (d.t == "interface") || (d.t == "p2p_interface") || (d.t == "svi_interface"))) {
            this.settingsLink(ws, d.v, d.t, d.i, d.type, d.order, d.color, d.weight, d.height);
        }
        else if(d.t === "text") {
            this.settingsText(ws, d.v, d.i, d.text, d.height, d.py, d.color, d.text_align, 
                d.bg_type, d.bg_show, d.bg_color, d.border_show, d.border_color, d.border_width, d.bg_depth, d.rotation_x);
        }
        else if(d.t == "symbol") {
            this.settingsSymbol(ws, d);
        }
        else {
            this.logmessage("WS msg error.");
            ws.terminate();
    ...
    processMessage_delete(ws, d) {
        if(d.v == "L2" && d.t == "base") {
            ...
            this.deleteL2Joint(ws, d.et, d.i, d.pi);
        }
        else if(d.v == "L3" && d.t == "joint") {
            this.deleteL3Joint(ws, d.et, d.i, d.pi);
        }
        else if(d.t == "text") {
            this.deleteText(ws, d.v, d.i);
        }
        else if(d.t == "symbol") {
            this.deleteSymbol(ws, d.v, d.i);
        }
        else {
            this.logmessage("WS msg error.");
            ws.terminate();
    ...
    processMessage_background(ws, d) {
        console.log("A")
    ...
    addSymbol(ws, d) {
        ...
        if (d.st === "A") {
            if(
                ...
                (! ["n", "f", "v", "p", "r", "s"].includes(d.cd.head_type)) ||
                (! ["n", "f", "v", "p", "r", "s"].includes(d.cd.tail_type)) ||
    ...
    moveElement(ws, view, type, id, px, py, pz, base) {
        if(
                (
                    ((view === "L2") && (type === "device")) ||
                    ((view === "L2") && (type === "text")) ||
    ...
    moveL2Joint(ws, et, id, joint_index, px, py, pz) {
        if (
                (et !== "link") ||
                (! (typeof px === "number")) ||
                (! (typeof py === "number")) ||
    ...
    moveL3Joint(ws, et, id, joint_index, px, py, pz) {
        if (
                (! (typeof px === "number")) ||
                (! (typeof py === "number")) ||
                (! (typeof pz === "number")) ||
    ...
    rotateElement(ws, view, type, id, rx, ry, rz) {
        // Verify Parameters
        if(
                (
                    ((view === "L2") && (type === "base")) ||
    ...
    resizeElement(ws, view, type, id, sx, sy, sz) {
        if(
                (
                    ((view === "L2") && (type === "base")) ||
                    ((view === "L2") && (type === "device")) ||
    ...
    settingsBase(ws, view, id, name, subtype, color1, color2, opacity, t1name, t2name, sy, tsx, tsy) {
        // Check parameters
        if (
                (! (typeof name === "string")) ||
                (name.length > 128) ||
    ...
    settingsVrf(ws, id, color1, color2) {
        // Check parameters
        if (
                (! (typeof color1 === "number")) ||
                (! (typeof color2 === "number")) ||
    ...
    settingsL2Segment(ws, id, color1) {
        // Check parameters
        if (
                (! (typeof color1 === "number")) ||
    ...
    settingsLink(ws, view, type, id, link_type, order, color, weight, height) {
        if (
    ...
    settingsText(ws, view, id, text, height, py, color, text_align, bg_type, bg_show, bg_color, border_show, border_color, border_width, bg_depth, rotation_x) {
        if((view !== "L2") && (view !== "L3")) {
            wsSendError(ws, "Invalid format");
            return;            
        }
    ...
    settingsSymbolFlag(ws, view, id, color, flagcolor) {
        if (
                (! (typeof color === "number")) ||
    ...
    settingsSymbolArrow(ws, d) {
        if (
                (! (typeof d.color === "number")) ||
                (! (typeof d.sx === "number")) ||
                (! (typeof d.sz === "number")) ||
                (typeof d.head_color !== "number") ||
                (! ["n", "f", "v", "p", "r", "s"].includes(d.head_type)) ||
                (! ["n", "f", "v", "p", "r", "s"].includes(d.tail_type)) ||
    ...
    settingsSymbol(ws, d) {
        ...
        if (
                (! (d.i in this.diagram[d.v].symbol))
            ) {
            wsSendError(ws, "Invalid format");
            return; 
        }
        if(this.diagram[d.v].symbol[d.i].type == "F")
            this.settingsSymbolFlag(ws, d.v, d.i, d.color, d.flagcolor);
        else if(this.diagram[d.v].symbol[d.i].type == "A")
            this.settingsSymbolArrow(ws, d);
    }
    ...
    deleteL3Base(ws, id) {
        if(id in this.diagram.L3.base) {
            ...
            if(!is_empty) {
                wsSendError(ws, "Base Element is not empty.");
                return;
            }
            if(Object.keys(this.diagram.L3.base).length < 2) {
                wsSendError(ws, "Can't remove last base element.");
                return;                
            }
    ...
    deleteL2Joint(ws, et, link_id, point_index) {
        if(
            (et !== "link") ||
    ...
    deleteL3Joint(ws, et, link_id, point_index) {
        if(
            (typeof point_index === "number") &&
            (typeof link_id === "string") &&
            (["l2link", "interface", "svi_interface", "p2p_interface"].indexOf(et) !== -1) ||                
            (link_id in this.diagram.L3[et]) &&
            (point_index < this.diagram.L3[et][link_id].linedata.points.length) &&
            (point_index >= 0)
            ) {
            this.diagram.L3[et][link_id].linedata.points.splice(point_index, 1);
