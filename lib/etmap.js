...
class ETMap {
    ...
    fix_diagram_version() {
        ...
        if(!("bg_color" in this.diagram.settings)) this.diagram.settings.bg_color = 0xf0f0f0;
        ...
        if(!("bgp_peering" in this.diagram.L3)) this.diagram.L3.bgp_peering = {};
        if(!("symbol" in this.diagram.L3)) this.diagram.L3.symbol = {};
        if(!("text" in this.diagram.L3)) this.diagram.L3.text = {};
    ...
    sendInitializeMessage(ws, msg_id) {
        ws.send(JSON.stringify({
                m: "I", 
                d: {n: this.name, d: this.diagram, p: ws.nm_data.permission},
    ...
    processMessage_settings(ws, msg_id, d) {
        if(((d.v === "L2") || (d.v === "L3")) && d.t == "base") {
            this.settingsBase(ws, msg_id, d.v, d.i, d.name, d.subtype, d.color1, d.color2, d.opacity, d.t1name, d.t2name, d.sy, d.tsx, d.tsy);
        }
        else if(d.v == "L2" && d.t == "device") {
            this.settingsL2Device(ws, msg_id, d.i, d.name, d.color1, d.color2, d.ifnaming);
        }
        else if(d.v == "L2" && d.t == "link") {
            this.settingsLink(ws, msg_id, d.v, d.t, d.i, d.type, d.order, d.color, d.weight, d.height);
        }
    ...
    addL3BGPPeer(ws, msg_id, d) {
        if(
            (["p2p", "lo2lo", "svi2svi", "svi2if", "svi2range", "if2if", "if2range"].indexOf(d.type) === -1) ||
            (["ipv4", "ipv6"].indexOf(d.transport) === -1) ||
            (!Array.isArray(d.afisafi))
            ) {
            wsSendError(ws, msg_id, "Invalid type, transport or afi/safi");
            return;
        }

        // Check transport IPs
        if((d.transport === "ipv4") && (d.type !== "svi2range") && (d.type !== "if2range") && ((!net.isIPv4Address(d.src_ip)) || (!net.isIPv4Address(d.dst_ip)))) {
            wsSendError(ws, msg_id, "Transport IP addresses are not IPv4 (1)");
            return;
        }
        else if((d.transport === "ipv4") && ((net.getIPv4Interface(d.src_ip) !== undefined) || (net.getIPv4Interface(d.dst_ip) !== undefined))) {
            wsSendError(ws, msg_id, "Transport IP addresses are not IPv4 (2)");
            return;
        }
        else if((d.transport === "ipv6") && (d.type !== "svi2range") && (d.type !== "if2range") && ((!net.isIPv6Address(d.src_ip)) || (!net.isIPv6Address(d.dst_ip)))) {
            wsSendError(ws, msg_id, "Transport IP addresses are not IPv6 (1)");
            return;
        }
        else if((d.transport === "ipv6") && ((net.getIPv6Interface(d.src_ip) !== undefined) || (net.getIPv6Interface(d.dst_ip) !== undefined))) {
            wsSendError(ws, msg_id, "Transport IP addresses are not IPv6 (2)");
            return;
        }
        ...
        // For each type, check it's L3 references. If all are good, create object and return.
        if(d.type === "p2p") {
        }
        else if(d.type === "lo2lo") {
            if(
                    (!(d.src_vrf_id in this.diagram.L3.vrf)) ||
                    (!(d.dst_vrf_id in this.diagram.L3.vrf)) ||
                    (!("los" in this.diagram.L3.vrf[d.src_vrf_id])) ||
                    (!("los" in this.diagram.L3.vrf[d.dst_vrf_id])) ||
                    (!(d.src_lo_id in this.diagram.L3.vrf[d.src_vrf_id].los)) ||
                    (!(d.dst_lo_id in this.diagram.L3.vrf[d.dst_vrf_id].los))
                ) {
                wsSendError(ws, msg_id, "SRC/DST vrf or referenced Loopbacks don't exist.");
                return;
            }
            // Create the bgp peering.
            new_bgppeer.l3_reference = {
                src_vrf_id: d.src_vrf_id,
                dst_vrf_id: d.dst_vrf_id,
                src_lo_id: d.src_lo_id,
                dst_lo_id: d.dst_lo_id,
            };
        }
        else if(d.type === "svi2svi") {
            if(
                    (!(d.p2p_interface_id in this.diagram.L3.p2p_interface))
                ) {
                wsSendError(ws, msg_id, "Invalid p2p interface.");
                return;
            }
            new_bgppeer.l3_reference = {
                p2p_interface_id: d.p2p_interface_id,
            };
        }
        else if(d.type === "svi2if") {
            if(
                    (!(d.src_svi_interface_id in this.diagram.L3.svi_interface)) ||
                    (!(d.dst_interface_id in this.diagram.L3.interface))
                ) {
                wsSendError(ws, msg_id, "Invalid svi or interface.");
                return;
            }
            new_bgppeer.l3_reference = {
                src_svi_interface_id: d.src_svi_interface_id,
                dst_interface_id: d.dst_interface_id,
            };
        }
        else if(d.type === "svi2range") {
            if(
                    (!(d.src_svi_interface_id in this.diagram.L3.svi_interface))
                ) {
                wsSendError(ws, msg_id, "Invalid svi interface.");
                return;
            }
            new_bgppeer.l3_reference = {
                src_svi_interface_id: d.src_svi_interface_id,
        ...
        else if(d.type === "if2if") {
            if(
                    (!(d.src_interface_id in this.diagram.L3.interface)) ||
                    (!(d.dst_interface_id in this.diagram.L3.interface))
                ) {
                wsSendError(ws, msg_id, "Invalid src or dst interface.");
                return;
            }
            new_bgppeer.l3_reference = {
                src_interface_id: d.src_interface_id,
                dst_interface_id: d.dst_interface_id,
            };
        }
        else if(d.type === "if2range") {
            if(
                    (!(d.src_interface_id in this.diagram.L3.interface))
                ) {
                wsSendError(ws, msg_id, "Invalid src interface.");
                return;
            }
            new_bgppeer.l3_reference = {
                src_interface_id: d.src_interface_id,
    ...
    settingsBase(ws, msg_id, view, id, name, subtype, color1, color2, opacity, t1name, t2name, sy, tsx, tsy) {
        // Check parameters
        if (
                (! (typeof name === "string")) ||
                (name.length > 128) ||
        ...
        this.broadcastMessage(ws, msg_id, {
            ...
            d: {
                ...
                t: "base",
                i: id,
                name: name, subtype: subtype, color1: color1, color2: color2, opacity: opacity, t1name: t1name, t2name: t2name, sy:sy, tsx: tsx, tsy: tsy
            }
        })
    }

    settingsL2Device(ws, msg_id, id, name, color1, color2, ifnaming) {
        // Check parameters
        if (
                (! (typeof name === "string")) ||
                (name.length > 128) ||
    ...
    settingsLink(ws, msg_id, view, type, id, link_type, order, color, weight, height) {
        if (
    ...
    configVRF(ws, msg_id, d) {
        ...
        // Update l3 diagram. This is needed as loopback interfaces might have been
        // changed which might affect bgp peering.
        let l3_changes = l3processor.update_from_l2(this.diagram);
        message.l3_changes = l3_changes;
