...
function wsSendError(ws, msg_id, message) {
    let response = {
        m: "E",
        d: message,
        msg_id: msg_id,
    ...
    configL2Device(ws, msg_id, d) {
        ...
        for(let svi_tag in d.svis) {
            ...
            for(let x = 0; x < svi.ipv4.length; x++) {
                if(net.getIPv4Interface(svi.ipv4[x]) === undefined) {
                    wsSendError(ws, msg_id, "Invalid format (svis ipv4)");
                    return;
                }
            }
            for(let x = 0; x < svi.ipv6.length; x++) {
                if(net.getIPv6Interface(svi.ipv6[x]) === undefined) {
                    wsSendError(ws, msg_id, "Invalid format (svis ipv6)");
                    return; 
                }
            }
            // Check vrfs
        ...
        for(let lo_id in d.los) {
            ...
            if(
                ...
                wsSendError(ws, msg_id, "Invalid format (loopback)");
                console.log(d);
                return;
            }
            ...
            for(let x = 0; x < lo.ipv4.length; x++) {
                if(net.getIPv4Interface(lo.ipv4[x]) === undefined) {
                    wsSendError(ws, msg_id, "Invalid format (loopbacks ipv4)");
                    return;
                }
            }
            for(let x = 0; x < lo.ipv6.length; x++) {
                if(net.getIPv6Interface(lo.ipv6[x]) === undefined) {
                    wsSendError(ws, msg_id, "Invalid format (loopbacks ipv6)");
                    return; 
                }
            }
            ...
            // Check vrfs
            if(!(lo.vrf in d.vrfs)) {
                wsSendError(ws, msg_id, "Invalid format (loopbacks vrf)");
            }
        ...
        if(d.function === "none") {
            ...
            for(let x = 0; x < d.subinterfaces.length; x++) {
                ...
                if(!Array.isArray(subif.ipv4))
                    return wsSendError(ws, msg_id, "Invalid IPv4 List");
                for(let y = 0; y < subif.ipv4.length; y++) {
                    if(net.getIPv4Interface(subif.ipv4[y]) === undefined)
                        return wsSendError(ws, msg_id, "Invalid IPv4: " + subif.ipv4[y]);
                }

                if(!Array.isArray(subif.ipv6))
                    return wsSendError(ws, msg_id, "Invalid IPv6 List");
                for(let y = 0; y < subif.ipv6.length; y++) {
                    if(net.getIPv6Interface(subif.ipv6[y]) === undefined)
                        return wsSendError(ws, msg_id, "Invalid IPv6: " + subif.ipv6[y]);
                }
