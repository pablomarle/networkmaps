...
class ETMap {
    constructor(name, path, uuid, usermgt, savediagram_interval, destroycallback) {
        this.name = name;
        this.path = path;
        this.uuid = uuid;
        this.diagram = null;
    ...
    static loadDiagram(path, uuid, callback) {
        fs.readFile(path + "/" + uuid + ".json", "utf-8", (err, data) => {
            if(err) {
                // Load default diagram
                callback(null, ETMap.defaultDiagram());
                console.log("Loaded default diagram. Couldn't find " + this.path + "/" + this.uuid + ".json");
            }
            else {
                try {
    ...
    loadDefault() {
        this.diagram = ETMap.defaultDiagram();
    }
    ...
    initialize(callback) {
        ...
        ETMap.loadDiagram(this.path, this.uuid, (err, data) => {
            if(err) {
                callback("Parse Error");
                return;
            }
