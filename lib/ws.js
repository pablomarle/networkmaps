...
function exportDiagram(sessionid, uuid, callback) {
    usermgt.isUserAllowed(sessionid, uuid, (error, user_result) => {
        if(error) {
            ...
            if(!(uuid in etmaps)) {
                ETMap.loadDiagram(config.diagrams.path, uuid, (err, diagram_data) => {
                    if(err)
                        callback(err);
                    else
                        callback(null, {
...
function WS_DIAGRAM(ws, uuid, sessionid) {
    ...
    usermgt.isUserAllowed(sessionid, uuid, (error, result) => {
        ...
        if(!(uuid in etmaps)) {
            etmaps[uuid] = new ETMap(result.ddata.name, config.diagrams.path, uuid, usermgt, config.timers.savediagram, (uuid) => {
                delete etmaps[uuid];
            })
...
function WS_DIAGRAM_CREATE(ws, sessionid, data) {
    usermgt.createDiagram(sessionid, data.n, (error, uuid) => {
        if(error) {
            ws.send(JSON.stringify({
                m: "DN", 
                d: { error: error}
