...
function fill_default_settings(initial_settings) {
	...
	if(initial_settings) {
		...
		if("staticserver" in initial_settings) {
			change_if_exists(settings["staticserver"], initial_settings["staticserver"], "hostname", settings["socket"]["address"]);
			change_if_exists(settings["staticserver"], initial_settings["staticserver"], "port", settings["socket"]["port"]);
		}
		else {
			settings["staticserver"]["hostname"] = settings["socket"]["address"];
			settings["staticserver"]["port"] = settings["socket"]["port"];			
		}
