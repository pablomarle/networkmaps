...
function handleWSConnection(ws, sessionid, router, isAdmin) {
    ...
    let sessiondata = usermgt.getSession(sessionid);
    if("error" in sessiondata) {
        clearInterval(ws.interval);
        ws.close();
        wsLogger.error(sessiondata.error);
        return;
    }
