        ...
        for(let bgp_id in diagram.L3.bgp_peering) {
            ...
            if(bgp.type === "lo2lo") {
                if(
                        ...
                        (!("los" in diagram.L3.vrf[bgp.l3_reference.src_vrf_id])) ||
                        (!("los" in diagram.L3.vrf[bgp.l3_reference.dst_vrf_id])) ||
                        (!(bgp.l3_reference.src_lo_id in diagram.L3.vrf[bgp.l3_reference.src_vrf_id].los)) ||
                        (!(bgp.l3_reference.dst_lo_id in diagram.L3.vrf[bgp.l3_reference.dst_vrf_id].los))
                    )
                    to_delete.push(bgp_id);
            }
            else if(bgp_type === "p2p") {
                if(!(bgp.l3_reference.p2p_interface_id in diagram.L3.p2p_interface))
                    to_delete.push(bgp_id);
            }
            else if(bgp_type === "svi2svi") {
                if(
                        (!(bgp.l3_reference.src_svi_interface_id in diagram.L3.svi_interface)) ||
                        (!(bgp.l3_reference.dst_svi_interface_id in diagram.L3.svi_interface))
                    )
                    to_delete.push(bgp_id);
            }            
            else if(bgp_type === "svi2if") {
                if(
                        (!(bgp.l3_reference.src_svi_interface_id in diagram.L3.svi_interface)) ||
                        (!(bgp.l3_reference.dst_interface_id in diagram.L3.interface))
                    )
                    to_delete.push(bgp_id);
            }
            else if(bgp_type === "svi2range") {
                if(
                        (!(bgp.l3_reference.src_svi_interface_id in diagram.L3.svi_interface))
                    )
                    to_delete.push(bgp_id);
            }
            else if(bgp_type === "if2if") {
                if(
                        (!(bgp.l3_reference.src_interface_id in diagram.L3.interface)) ||
                        (!(bgp.l3_reference.dst_interface_id in diagram.L3.interface))
                    )
                    to_delete.push(bgp_id);
            }
            else if(bgp_type === "if2range") {
                if(
                        (!(bgp.l3_reference.src_interface_id in diagram.L3.interface))
                    )
                    to_delete.push(bgp_id);
