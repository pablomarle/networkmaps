...
const {fix_diagram_version} = require('./migrations/etmap_migrations');
const validator = require('validator');
const { Logger } = require('../utils/logger');
const { fullWidth } = require('validator/lib/isFullWidth');

const etmapLogger = new Logger({ prefix: 'ETMap' });
...
class ETMap {
    ...
    static saveDiagramSync(path, uuid, diagram) {
        try {
            fs.writeFileSync(path + "/" + uuid + ".json", JSON.stringify(diagram));
            return null;
        }
        catch(err) {
            return "Error saving diagram: " + err;
        }
    }
    ...
    static loadDiagram(type, path, uuid, callback) {
        fs.readFile(path + "/" + uuid + ".json", "utf-8", (err, data) => {
            if(err) {
                ...
                etmapLogger.info(`Loaded default diagram of type ${type}. Couldn't find ${path}/${uuid}.json`);
                ...
                try {
                    ...
                    etmapLogger.error(`Error parsing diagram ${path}/${uuid}.json: ${err}`);
    ...
    logmessage(message) {
        etmapLogger.info(`UUID: ${this.uuid} C: ${this.clients.length} M: ${message}`);
    }
    ...
    initialize(callback) {
        ...
        ETMap.loadDiagram(this.type, this.path, this.uuid, (err, data) => {
            ...
            let migrations = fix_diagram_version(this.diagram);
            migrations.forEach((entry) => {
                etmapLogger.info(`Migration on ${this.uuid}: ${entry.from} => ${entry.to}`)
            });
    ...
    /**
     * Function to close the diagram and save it to disk. It will also close all the websocket connections
     * and call the callback function to remove the diagram from the list of active diagrams.
     * Compared to destroy, this function is expected to be used when the daemon is shutting down.
     */
    shutdown() {
        ...
        // Save diagrams
        this.logmessage("Saving diagram before shutdown");
        let err = ETMap.saveDiagramSync(this.path, this.uuid, this.diagram);
        if(err)
            this.logmessage(err);
    ...
    setupWSCalls(ws) {
        ...
        ws.on("error", (error) => {
            etmapLogger.error("WS ERROR on diagram:");
            etmapLogger.error(error);
            clearInterval(ws.interval);
            this.removeWS(ws);
        })
    ...
    processMessage_shapes(ws, msg_id, d) {
        if(d.m === "A") {
            ...
            if(index !== -1) {
                ...
                let data = this.usermgt.listShapes(ws.nm_data.sessionid);
                if("error" in data) {
                    wsSendError(ws, msg_id, data.error);
                    return;
                }
                if(!(d.id in data)) {
