...
class UserMGT {
    ...
    getSession(sessionid) {
        ...
        if(sessionid in this.data.session) {
            ...
            return {
                "sessionid": sessionid,
                "data": this.data.session[sessionid].data
            };
    ...
    getSessionNoCreate(sessionid) {
        if (!validator.isAlphanumeric(sessionid)) {
            return { error: "Error on get session (3): '" + sessionid + "'" };
        }
        ...
        if(sessionid in this.data.session) {
            ...
            return { error: "Error on get session (2)." };
        }
    }
    ...
    createUser(userdata) {
        ...
        if(!this.isUserDomainAllowed(userdata.email)) {
            return {
                error: "Domain not allowed to register"
            };
        }
    ...
    validateUser(email, validate_string) {
        if((!this.register_self) || (this.authentication !== "local")) {
            return {"error": "Users not allowed to register."};
        }
        ...
        if (!validator.isAlphanumeric(validate_string)) {
            return {"error": "Activation code invalid"};
        }
        ...
        if((email in this.data.user) && (!this.data.user[email].is_active) && (this.data.user[email].activation_code === validate_string)) {
            ...
            return {"error": "Error on user activation (3)"};
        }
    }

    changePassword(email, oldpassword, newpassword) {
        if(this.authentication !== "local") {
            return { success: false, error: "You can't change your password here (" + this.authentication + " authentication)." };
        }
        ...
        if((email in this.data.user) && (this.data.user[email].is_active) && (!this.data.user[email].mark_delete)) {
            ...
            return { success: false, error: "Error on changePassword" };
        }
    }

    requestResetPassword(email) {
        if(this.authentication !== "local") {
            return { error: "Can't reset password here (" + this.authentication + ")." };
        }
    ...
    async loginUser(sessionid, email, password) {
        ...
        if(!this.isUserDomainAllowed(email)) {
            return { error: "Domain not allowed to register" };
        }
        ...
        if(this.authentication === "local") {
            ...
            if((email in this.data.user) && (this.data.user[email].is_active) && (!this.data.user[email].mark_delete)) {
                ...
                if (hash === this.data.user[email].password) {
                    ...
                    return this.data.session[sessionid].data;
                }
                else
    ...
    changeUserData(sessionid, newusername, newuserlastname) {
        if(this.authentication !== "local") {
            return { error: "Can't change user data here (" + this.authentication + ")." };
        }
        ...
        if(sessionid in this.data.session) {
            ...
            this.data.session[sessionid].data.name = newusername;
            this.data.session[sessionid].data.lastname = newuserlastname;
        ...
        if(this.authentication === "local") {
            ...
            if(email in this.data.user) {
                ...
                if("error" in createUserResult) {
                    callback("Error sharing diagram: " + createUserResult.error);
                    return;
                }
