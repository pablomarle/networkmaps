...
class UserMGT {
    constructor(path, user_timeout, save_timeout, register_self, admin_user, admin_password, allowed_domains) {
        this.filename = path + "/user_db.json";
        this.data = {
            session: {},
            user: {},
            password_reset: {},
        ...
        this.register_self = register_self;
        this.admin_user = admin_user;
        this.admin_password = admin_password;
        this.allowed_domains = (Array.isArray(allowed_domains)) ? allowed_domains : [];
    }
        ...
        this.cleanup_intervale = setInterval(() => {
            ...
            for(let username in this.data.user) {
                if(!this.data.user[username].is_active && this.data.user[username].activation_date < timestamp) {
                    for(diagram_uuid in this.data.user[username].diagrams) {
    ...
    createUser(userdata, callback) {
        ...
        if(this.allowed_domains.length > 0) {
            ...
            let user_domain = userdata.email.split("@")[1];
