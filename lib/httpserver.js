...
class HTTPServer {
     ...
     * @param {int} session_timeout - Session timeout in seconds
     */
    constructor(use_ssl, hostname, port, certpath, keypath, requestcallback, wsconnectcallback, session_timeout) {
    ...
    handleResponse(response, statusCode, contentType, content, newsessionid, redirectLocation, cacheMaxAge) {
        ...
        if(newsessionid)
            if(this.use_ssl)
                response.setHeader('Set-Cookie', 'NetSessionSec=' + newsessionid + "; Max-Age=" + this.session_timeout + "; path=/; secure");
            else
                response.setHeader('Set-Cookie', 'NetSessionNoSec=' + newsessionid + "; Max-Age=" + this.session_timeout + "; path=/");
