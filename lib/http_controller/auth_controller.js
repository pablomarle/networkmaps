...
function showIndex(callData) {
    ...
    if ((context.config.users.authentication === "openid") && (!session.data.user)) {
        let result = context.usermgt.initOpenidAuth(session.sessionid, context.html.get_server());
        if("error" in result) {
            sendresponse(403, "text/html", context.html.notAuthorized(result.error), session.sessionid);
            return;
        }
        sendresponse(302, null, "", session.sessionid, result.redirect_url);
    } else {
...
function handleOpenIdCallback(callData) {
    ...
    context.usermgt.authOpenid(session.sessionid, url.split("?")[1], context.html.get_server(), (err) => {
        if (err) {
