...
function listShapes(callData) {
    ...
    let result = context.usermgt.listShapes(session.sessionid);
    if ("error" in result) {
        sendresponse(401, "application/json", JSON.stringify({error: result.error}), session.sessionid);
        return;
    }
 ...
 * @param {Object} callData - Object containing request details
 */
async function newShapeGroup(callData) {
    ...
    let result = await context.usermgt.newShape(
        session.sessionid,
        new_data.name,
    ...
    if ("error" in result) {
        sendresponse(200, "application/json", JSON.stringify({error: result.error}), session.sessionid);
        return;
    }
    sendresponse(200, "application/json", JSON.stringify(result), session.sessionid);
}
